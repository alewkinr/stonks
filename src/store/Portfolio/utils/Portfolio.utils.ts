...
const _filterBondsByInstrumentId = (bonds, _id: string): Array<Bond> => {
  return bonds.filter(
    (bond) => bond.instrumentId === _id
  )
}

// FindBondByInstrumentId - получаем объект бонда по его instrumentId
export const FindBondByInstrumentId = (bonds: { [instrumentId: string]: Bond }, instrumentId: string): Bond => {
...
const _hasBondIdAsKey = (bondsStoreObj: { [instrumentId: string]: any }, _id: string): boolean => {
